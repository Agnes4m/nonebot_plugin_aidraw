...
```bash
# .env 文件
draw_api_url = "https://api.openai.com/v1"   # 自定义中转站 base URL，必须以 /v1 结尾
draw_api_key = "your-api-key"                     # API 密钥
draw_model = "agnes-image-2.1-flash"              # 模型名称
```
...
```bash
draw_backend = "openai"   # openai / gemini / sd；仅在 draw_api_url 留空时生效
```
...
- 发送图片后输出本次 token 用量与模型（仅当有有效 tokens 时）
